...
O seletor de instrumento preserva uma sessão independente para o paquímetro e para o micrômetro interno. No micrômetro, a vista geral apresenta duas pontas finas verticais — uma fixa e uma móvel —, corpo horizontal, bainha descendente, tambor centesimal cônico, recartilha e catraca visual, seguindo a família analógica tipo paquímetro para capacidades iniciadas em 5 mm. A lupa centra a costura bainha/tambor. A geometria comunica leitura e abertura nominal sem afirmar que simula força, alinhamento ou a cinemática interna real.

O micrômetro externo preserva a bigorna fixa, o fuso móvel, a trava, a bainha, o tambor cônico, a empunhadura recartilhada e a catraca como volumes distintos. Sua faixa é `0–25 mm`, com seleção entre `0,01 mm` e `0,001 mm`; o nônio aparece no perfil milesimal. O arraste usa um passo da resolução selecionada por pixel na vista geral e na lupa. Quadro e componentes seguem os mesmos neutros metálicos, contornos e espessuras dos outros instrumentos, enquanto vinho permanece reservado à leitura e aos estados ativos. A geometria vigente da ferradura é um U quadrado com base e pernas retas, cantos inferiores arredondados e terminações que não invadem o batente esquerdo nem a circunferência da trava. A identificação Cabalero ocupa um letreiro retangular rebaixado, centralizado na base e com cantos de raio curto, sem rebites nem aparência de cartão de interface sobreposto.

O quarto instrumento ativo é um transferidor semicircular simples, com corpo fixo, faixa interativa de `5–180°`, divisões físicas de `1°`, base diametral, régua móvel e pivô central. O extremo esquerdo mostra `5`, os rótulos interiores preservam seus valores reais — inclusive `90°` no topo — e o extremo direito permanece `180°`; não existe offset global. O complementar usa `180° − leitura`. Ele não possui nônio. A visão geral prioriza a formação do ângulo correspondente e reduz rótulos em telas estreitas antes de permitir colisões. A lupa não amplia o instrumento inteiro: apresenta uma janela localizada que interpola cada grau em 12 divisões de `5′`, enfatiza `15′`, `30′` e `45′`, mantém o ponteiro central e mostra a leitura complementar. Arraste e teclado escrevem o mesmo estado inteiro em minutos de arco.
